# lb - A maubot plugin to subscribe to LB/Atom feeds.
        ...
        if not isinstance(date, datetime):
            try:
                date = datetime.strptime(
                    date, date_fmt_microseconds if "." in date else date_fmt)
            except ValueError:
                date = datetime.now()
...
class DBManager:
    ...
    async def get_feeds(self) -> list[Feed]:
        q = """
        SELECT id, url, title, link, next_retry, error_count,
               room_id, user_id, send_notice
    ...
    async def get_feeds_by_room(self, room_id: RoomID) -> list[tuple[Feed, UserID]]:
        q = """
        SELECT id, url, title, link, next_retry, error_count, user_id FROM feed
    ...
    async def get_entries(self, feed_id: int) -> list[Entry]:
        q = "SELECT feed_id, id, date, title, description, link FROM entry WHERE feed_id = $1"
    ...
    async def add_entries(self, entries: list[Entry], override_feed_id: int | None = None) -> None:
        ...
        columns = ("feed_id", "id", "date", "title", "description", "link")
        async with self.db.acquire() as conn:
            if self.db.scheme == Scheme.POSTGRES:
                await conn.copy_records_to_table("entry", records=records, columns=columns)
            else:
                q = (
                    "INSERT INTO entry (feed_id, id, date, title, description, link) "
                    "VALUES ($1, $2, $3, $4, $5, $6)"
                )
                await conn.executemany(q, records)

    async def get_feed_by_url(self, url: str) -> Feed | None:
        q = "SELECT id, url, title, link, next_retry, error_count FROM feed WHERE url=$1"
    ...
    async def create_feed(self, info: Feed) -> Feed:
        q = (
            "INSERT INTO feed (url, title, link, next_retry) "
        ...
        if self.db.scheme == Scheme.SQLITE:
            ...
            info.id = await self.db.fetchval(
                q, info.url, info.title, info.link, info.next_retry
            )
        return info
    ...
    async def subscribe(
        ...
        q = """
        INSERT INTO subscription (feed_id, room_id, user_id, send_notice)
        VALUES ($1, $2, $3, $4)
        """
        await self.db.execute(q, feed_id, room_id, user_id, send_notice)
